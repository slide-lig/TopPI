 ...
 *
 *
 *	Minor modifications at Northwestern University by
 *	Philip R. "Pib" Burns.  2004/08/10.
 *
 * -------------------------------------------------------------------------
 */
...
public class Complex implements java.io.Serializable, Cloneable
{
	...
	public static Complex asin(Complex z)
	{
		...
		if (Double.isInfinite(r)) {
			...
			result.re = 0;
			result.im = ArithUtils.asinh(z.im);
		} else if (r <= 0.1) {
	...
	{
		if (z.im == 0) {
			return new Complex(ArithUtils.cosh(z.re));
		}

		double	coshx = ArithUtils.cosh(z.re);
		double	sinhx = ArithUtils.sinh(z.re);
	 ...
	 *	@return	The imaginary part of the current complex object.
	 */
	...
	public static Complex sinh(Complex z)
	{
		...
		if (z.im == 0) {
			result = new Complex(ArithUtils.sinh(z.re));
		} else {
			// A&S 4.5.49
	...
	{
		double	sinh2x = ArithUtils.sinh(2*z.re);

		if (z.im == 0) {
			return new Complex(ArithUtils.tanh(z.re));
		} else if (sinh2x == 0) {
			return new Complex(0,Math.tan(z.im));
		}

		double	cosh2x = ArithUtils.cosh(2*z.re);
