 ...
 * only items having a support count in [minusp, 100% [ - item ordering in
 * transactions (in ascending order) - occurence delivery (actually, filtered
 * transactions are only referenced in occurences lists) - fast
...
public class BasicDataset extends Dataset {
	...
	/**
	 * frequent item => List<transactions containing item> In other words, this
	 * map provides projections "for free"
	 * 
	 * Note that transactions are added in the same order in all
	 * occurences-ArrayLists. This property is used in CandidatesIterator's
	 * prefix-preserving test
	 */
	...
	protected BasicDataset(final int minimumsupport,
		...
		// coreItem is known to have a 100% support, we don't want it in the
		// computed closure
	...
	protected class CandidatesIterator implements ExtensionsIterator {
		...
		private boolean prefixPreservingTest(final int candidateIndex) {
			...
			for (int i = candidateIndex + 1; i < frequentItems.length; i++) {
				...
				// otherwise we have supp(j) < supp(candidate) : no need to
				// worry about j
