...
public class VIntConcatenatedUnfilteredDataset extends Dataset {
	public static final double FILTERING_THRESHOLD = 0.15;

	protected final VIntConcatenatedDataset parent;
	 ...
	 */
	public VIntConcatenatedUnfilteredDataset(
			VIntConcatenatedDataset parentDataset, int extension)
			throws DontExploreThisBranchException {
	...
	public VIntConcatenatedUnfilteredDataset(
			VIntConcatenatedUnfilteredDataset upper, int extension,
	...
	private void genSupportCounts() {
		...
		while (iterator.hasNext()) {
			...
			IntHolder transIndex = new IntHolder(tid);
			int length = VIntConcatenatedDataset.readVInt(
					this.parent.concatenated, transIndex);
			transIndex.value = tid - length;
			while (transIndex.value < tid) {
				this.supportCounts.adjustOrPutValue(VIntConcatenatedDataset
						.readVInt(this.parent.concatenated, transIndex), 1, 1);
			}
		}
	}
	...
	public Dataset getProjection(int extension)
		...
		if ((extensionSupport / this.parent.getTransactionsCount()) > FILTERING_THRESHOLD) {
			...
			return new VIntConcatenatedDataset(this.parent, extension,
	...
	private TIntArrayList buildExtensionTIDs(int extension) {
		...
		TIntIterator parentExtTidsIt = this.parent.occurrences.get(extension)
				.iterator();
