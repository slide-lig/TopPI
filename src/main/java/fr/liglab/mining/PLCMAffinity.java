...
public class PLCMAffinity extends PLCM {
	...
	void createThreads(int nbThreads) {
		...
		this.threadsBySocket = new TopLCMAffinityThread[usedSockets][];
		
		for (int s = 0; s < usedSockets; s++) {
			TopLCMAffinityThread[] onThisSocket = new TopLCMAffinityThread[Math.min(corePerSocket, remainingThreads)];
			...
			for (int c = 0; c < corePerSocket && remainingThreads > 0; c++, id++, remainingThreads--) {
				TopLCMAffinityThread t = new TopLCMAffinityThread(id, s, socketLocks[s], bindingSem, (c != 0));
				onThisSocket[c] = t;
				this.threads.add(t);
			}
		}
	...
	void initializeAndStartThreads(final ExplorationStep initState) {
		...
		for (int socketId = 0; socketId < this.threadsBySocket.length; socketId++) {
			if (socketId % this.copySocketModulo == 0) {
				if (socketId == 0) {
					...
					TopLCMAffinityThread t = this.threadsBySocket[socketId][0];
					t.init(initState);
					t.start();
				} else {
					nbCopies++;
		...
		for (TopLCMThread thread : this.threads) {
			if (!thread.isAlive()) {
				thread.start();
			}
		}
	...
	@Override
	public ExplorationStep stealJob(final TopLCMThread t) {
		TopLCMAffinityThread thief = (TopLCMAffinityThread) t;
		
		/// FIXME this will only work if copySocketModulo is ==1 or ==usedSockets
		
		for (TopLCMAffinityThread fellow : this.threadsBySocket[thief.socketId]) {
			if (fellow == thief) {
				continue;
			}
		...
		for (int socketId = 0; socketId < this.threadsBySocket.length; socketId++) {
			if (socketId != thief.socketId) {
				for (TopLCMAffinityThread victim : this.threadsBySocket[socketId]) {
	...
	private class TopLCMAffinityThread extends TopLCMThread {
		...
		private TopLCMAffinityThread(int id, int socket, AffinityLock al, Semaphore bindingSem, boolean getNewAL) {
			super(id);
			this.al = al;
