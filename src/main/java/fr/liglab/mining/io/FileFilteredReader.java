...
/**
 * FileReader, copied, applying a renaming as soon as the first pass and
 * filtering per group
 */
public final class FileFilteredReader implements Iterator<TransactionReader> {
	...
	private void prepareNextCopyReader() {
		if (currentPage == null || currentTransIdx == COPY_PAGES_SIZE || currentPage[currentTransIdx] == -1) {
		...
		if (renaming != null) {
			...
			if (filteredI == currentTransIdx) { // completely filtered
