...
public class PerItemTopKCollectorTest {
	...
	public void testEmpty() {
		...
		PerItemTopKCollector collector = new PerItemTopKCollector(stub, 50,
				true);
		collector.close();
	}
	...
	public void testCompleteLCM() {
		...
		RebasedConcatenatedDataset dataset = new RebasedConcatenatedDataset(2,
				trans);
	...
	public void testExplore() {
		...
		assertTrue(collector.explore(new int[] { 2 }, 0, freqs_2, supports_2) == -1);
		collector.collect(3, new int[] { 2, 0 });

		// next one should be 2,{2,1} - which support is too low
		assertFalse(collector.explore(new int[] { 2 }, 1, freqs_2, supports_2) == -1);
	}
}
