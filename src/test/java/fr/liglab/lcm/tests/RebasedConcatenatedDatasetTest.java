...
public class RebasedConcatenatedDatasetTest {
	
	@Test
	public void testNoCrashOnEmpty() {
		// super-high minimum support
		Dataset dataset = new RebasedConcatenatedDataset(99999, FileReaderTest.getMicroReader());
		assertEquals(0, dataset.getDiscoveredClosureItems().length);
		assertFalse(dataset.getCandidatesIterator().hasNext());
		// OR empty file
		dataset = new RebasedConcatenatedDataset(1, new EmptyInputFile());
		assertEquals(0, dataset.getDiscoveredClosureItems().length);
		assertFalse(dataset.getCandidatesIterator().hasNext());
	}
	...
	public void testRemapping() {
		// rebasing.dat
		RebasedDataset dataset = new RebasedConcatenatedDataset(2, FileReaderTest.getRebasing());
		...
		// globalclosure.dat
		dataset = new RebasedConcatenatedDataset(2, FileReaderTest.getGlobalClosure());
		...
		dataset = new RebasedConcatenatedDataset(2, FileReaderTest.getFakeGlobalClosure());
	...
	public void testFileLoading() {
		Dataset dataset = new RebasedConcatenatedDataset(2, FileReaderTest.getRebasing());
